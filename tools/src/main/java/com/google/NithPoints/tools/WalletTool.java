...
package com.google.NithPoints.tools;
...
import com.google.NithPoints.discovery.PeerDiscovery;
import com.google.NithPoints.store.*;
...
/**
 * A command line tool for manipulating wallets and working with NithPoints.<p>
 */
public class WalletTool {
    ...
    public static class Condition {
        ...
        public boolean matchNithPointss(BigInteger comparison) {
            try {
    ...
    private static void send(List<String> outputs, BigInteger fee, String lockTimeStr, boolean allowUnconfirmed) {
        try {
            ...
            if (!wallet.completeTx(req)) {
                System.err.println("Insufficient funds: have " + Utils.NithPointsValueToFriendlyString(wallet.getBalance()));
                return;
            }
            try {
                if (lockTimeStr != null) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchNithPointss(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchNithPointss(balance)) {
                            System.out.println(Utils.NithPointsValueToFriendlyString(balance));
                            latch.countDown();
                        }
                    }
                });
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a NithPoints address of the right network parameters.");
                return;
            }            
        }
        if (key == null) {
