...
public class LazyParseByteCacheTest {
    ...
    public void setUp() throws Exception {
        ...
        NithPointsSerializer bs = new NithPointsSerializer(unitTestParams);
        
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        bs.serialize(tx1, bos);
        tx1BytesWithHeader = bos.toByteArray();
        tx1Bytes = tx1.NithPointsSerialize();
        
        bos.reset();
        bs.serialize(tx2, bos);
        tx2BytesWithHeader = bos.toByteArray();
        tx2Bytes = tx2.NithPointsSerialize();
        
        bos.reset();
        bs.serialize(b1, bos);
        b1BytesWithHeader = bos.toByteArray();
        b1Bytes = b1.NithPointsSerialize();
    }
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
    	...
    	//message structure.
    	NithPointsSerializer bsRef = new NithPointsSerializer(unitTestParams, false, false);
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
    	
    	NithPointsSerializer bs = new NithPointsSerializer(unitTestParams, lazy, retain);
    	Block b1;
    	Block bRef;
    	...
    	//verify our reference NithPointsSerializer produces matching byte array.
    	bos.reset();
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
    	...
    	//reference serializer to produce comparison serialization output after changes to
    	//message structure.
    	NithPointsSerializer bsRef = new NithPointsSerializer(params, false, false);
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
    	
    	NithPointsSerializer bs = new NithPointsSerializer(params, lazy, retain);
    	Transaction t1;
    	Transaction tRef;
    ...
    private void serDeser(NithPointsSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes, byte[] containingBytes) throws Exception {
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
