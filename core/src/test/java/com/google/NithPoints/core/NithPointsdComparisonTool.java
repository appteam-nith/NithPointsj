...
import com.google.NithPoints.store.FullPrunedBlockStore;
import com.google.NithPoints.store.MemoryFullPrunedBlockStore;
import com.google.NithPoints.utils.BriefLogFormatter;
...
/**
 * A tool for comparing the blocks which are accepted/rejected by NithPointsd/NithPointsj
 * It is designed to run as a testnet-in-a-box network between a single NithPointsd node and NithPointsj
 * It is not an automated unit-test because it requires a bit more set-up...read comments below
 */
public class NithPointsdComparisonTool {
    private static final Logger log = LoggerFactory.getLogger(NithPointsdComparisonTool.class);

    private static NetworkParameters params;
    private static FullPrunedBlockStore store;
    private static FullPrunedBlockChain chain;
    private static PeerGroup peers;
    private static Sha256Hash NithPointsdChainHead;
    private static Peer NithPointsd;
    
    public static void main(String[] args) throws Exception {
        BriefLogFormatter.init();
        new NithPointsdComparisonTool(args.length > 0 ? Integer.parseInt(args[0]) : 8333);
    }

    public NithPointsdComparisonTool(int port) throws Exception {
        params = NetworkParameters.testNet2();
        /**
         * The following have been changed from the default and do not match NithPointsd's default.
         * In order for this test to work, NithPointsd should be recompiled with the same values you see here.
         ...
         * 
         * You could also simply use git apply to apply the "NithPointsd-comparison.patch" file included with NithPointsj
         */
        ...
        // Only needs to be set in NithPointsj
        ...
        // NithPointsd MUST be on localhost or we will get banned as a DoSer
        ...
        peers.addEventListener(new AbstractPeerEventListener() {
            ...
            public void onPeerConnected(Peer peer, int peerCount) {
                ...
                log.info("NithPointsd connected");
                NithPointsd = peer;
            }
            ...
            public void onPeerDisconnected(Peer peer, int peerCount) {
                ...
                log.error("NithPointsd node disconnected!");
                System.exit(1);
            }
            ...
            public Message onPreMessageReceived(Peer peer, Message m) {
                if (m instanceof HeadersMessage) {
                    for (Block block : ((HeadersMessage) m).getBlockHeaders())
                        NithPointsdChainHead = block.getHash();
                    return null;
                }
                else if (m instanceof Block) {
                    log.error("NithPointsd sent us a block it already had, make sure NithPointsd has no blocks!");
                    System.exit(1);
                }
                return m;
            }
        ...
        NithPointsdChainHead = params.genesisBlock.getHash();
        
        // Connect to NithPointsd and make sure it has no blocks
        peers.start();
        peers.setMaxConnections(1);
        peers.downloadBlockChain();
        
        while (NithPointsd == null)
            Thread.sleep(50);
        ...
        for (BlockAndValidity block : blockList) {
            ...
            NithPointsd.sendMessage(block.block);
            locator.clear();
            locator.add(NithPointsdChainHead);
            NithPointsd.sendMessage(new GetHeadersMessage(params, locator, hashTo));
            NithPointsd.ping().get();
            if (!chain.getChainHead().getHeader().getHash().equals(NithPointsdChainHead)) {
                differingBlocks++;
                log.error("NithPointsd and NithPointsj acceptance differs on block \"" + block.blockName + "\"");
            }
            log.info("Block \"" + block.blockName + "\" completed processing");
        }
        
        log.info("Done testing.\n" +
        		"Blocks which were not handled the same between NithPointsd/NithPointsj: " + differingBlocks + "\n" +
