...
public class BlockTest {
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = new Block(params, header.NithPointsSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testNithPointsSerialization() throws Exception {
        ...
        //
        // NB: This tests the NithPoints proprietary serialization protocol. A different test checks Java serialization
        // of transactions.
        Block block = new Block(params, blockBytes);
        assertTrue(Arrays.equals(blockBytes, block.NithPointsSerialize()));
    }
    ...
    public void testJavaSerialiazation() throws Exception {
        ...
        // Note that this will actually check the transactions are equal by doing NithPoints serialization and checking
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.NithPointsSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.NithPointsSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.NithPointsSerialize().length, tx.length);
        ...
        assertEquals(block.length, block.NithPointsSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
