 ...
 */
public class NithPointsURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(NithPointsURI.class);
    ...
    public static final String NithPoints_SCHEME = "NithPoints";
    ...
    /**
     * Constructs a new NithPointsURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws NithPointsURIParseException if the URI is not syntactically or semantically valid.
     */
    public NithPointsURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws NithPointsURIParseException If the input fails NithPoints URI syntax and semantic checks.
     */
    public NithPointsURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new NithPointsURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  NithPoints:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form NithPoints://address?....
        // We support both until Ben fixes his code.

        // Remove the NithPoints scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : NithPoints:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("NithPoints://")) {
            schemeSpecificPart = input.substring("NithPoints://".length());
        } else if (input.startsWith("NithPoints:")) {
            schemeSpecificPart = input.substring("NithPoints:".length());
        } else {
            throw new NithPointsURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new NithPointsURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new NithPointsURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a NithPoints address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new NithPointsURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new NithPointsURIParseException("Malformed NithPoints URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new NithPointsURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The NithPoints Address from the URI
     */
     ...
     *         10,000,000 units is 1 PTS. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("NithPointsURI[");
        boolean first = true;
    ...
    public static String convertToNithPointsURI(Address address, BigInteger amount, String label, String message) {
        return convertToNithPointsURI(address.toString(), amount, label, message);
    }

    /**
     * Simple NithPoints URI builder using known good fields.
     *
     * @param address The NithPoints address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the NithPoints URI
     */
    public static String convertToNithPointsURI(String address, BigInteger amount, String label, String message) {
        ...
        builder.append(NithPoints_SCHEME).append(":").append(address);

        boolean questionMarkHasBeenOutput = false;

        if (amount != null) {
            builder.append(QUESTION_MARK_SEPARATOR).append(FIELD_AMOUNT).append("=");
            builder.append(Utils.NithPointsValueToPlainString(amount));
