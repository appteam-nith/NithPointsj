...
/**
 * A collection of various utility methods that are helpful for working with the NithPoints protocol.
 * To enable debug logging from the library, run with -DNithPointsj.logging=true on your command line.
 */
public class Utils {
    ...
    /** The string that prefixes all text messages signed using NithPoints keys. */
    public static final String NithPoints_SIGNED_MESSAGE_HEADER = "NithPoints Signed Message:\n";

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a NithPoints.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using NithPoints.
    ...
    /**
     * How many "nanocoins" there are in 0.01 NithPointss.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using NithPoints.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
     ...
     * standard procedure in NithPoints. The resulting hash is in big endian form.
     */
     ...
     */
    public static String NithPointsValueToFriendlyString(BigInteger value) {
    ...
    /**
     * <p>
     * Returns the given value as a plain string denominated in PTS.
     * The result is unformatted with no trailing zeroes.
     * For instance, an input value of BigInteger.valueOf(150000) nanocoin gives an output string of "0.0015" PTS
     * </p>
     * 
     * @param value The value in nanocoins to convert to a string (denominated in PTS)
     * @throws IllegalArgumentException
     *            If the input value is null
     */
    public static String NithPointsValueToPlainString(BigInteger value) {
        if (value == null) {
            throw new IllegalArgumentException("Value cannot be null");
        }
                
        BigDecimal valueInPTS = new BigDecimal(value).divide(new BigDecimal(Utils.COIN));
        return valueInPTS.toPlainString();
    }
     ...
     *
     * <tt><p>[24] "NithPoints Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        VarInt size = new VarInt(message.length());
        int totalSize = 1 + NithPoints_SIGNED_MESSAGE_HEADER.length() + size.getSizeInBytes() + message.length();
        byte[] result = new byte[totalSize];
        int cursor = 0;
        result[cursor++] = (byte) NithPoints_SIGNED_MESSAGE_HEADER.length();
        byte[] bytes = NithPoints_SIGNED_MESSAGE_HEADER.getBytes(Charset.forName("UTF-8"));
