 ...
 *
 * <p>ECKey also provides access to NithPoints-Qt compatible text message signing, as accessible via the UI or JSON-RPC.
...
public class ECKey implements Serializable {
    ...
    static {
        // All clients must agree on the curve to use by agreement. NithPoints uses secp256k1.
    ...
    /**
     * Construct an ECKey from an ASN.1 encoded private key. These are produced by OpenSSL and stored by the NithPoints
    ...
    /**
     * Output this ECKey as an ASN.1 encoded private key, as understood by OpenSSL or used by the NithPoints reference
     ...
     * how ECDSA signatures are represented when embedded in other data structures in the NithPoints protocol. The raw
    ...
    public static class ECDSASignature {
         ...
         * of the type used by NithPoints we have to encode them using DER encoding, which is just a way to pack the two
     ...
     * usually encoded using DER format, so you want {@link com.google.NithPoints.core.ECKey.ECDSASignature#toASN1()}
     ...
     * the {@link com.google.NithPoints.core.DumpedPrivateKey#toString()} method to get the string.
     *
     ...
     *
     * Because it is a critical failure if the private keys cannot be decrypted successfully (resulting of loss of all NithPointss controlled
