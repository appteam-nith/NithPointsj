...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the NithPoints proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion NithPoints protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = NithPointsSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom NithPoints protocol.
    ...
    /**
     * Should only used by NithPointsSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by NithPointsSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeNithPointsSerialize()}, which is safe to mutate.
    ...
    public byte[] NithPointsSerialize() {
        byte[] bytes = unsafeNithPointsSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the NithPoints wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeNithPointsSerialize() {
        ...
        try {
            NithPointsSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to NithPointsSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the NithPoints wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void NithPointsSerialize(OutputStream stream) throws IOException {
    ...
    void NithPointsSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented NithPointsSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so NithPointsSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */
