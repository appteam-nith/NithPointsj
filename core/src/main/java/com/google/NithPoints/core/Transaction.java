 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the NithPoints C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of NithPoints and have many powerful features. Read
 * <a href="http://code.google.com/p/NithPointsj/wiki/WorkingWithTransactions">"Working with transactions"</a> in the
 * documentation to learn more about how to use this class.</p>
 *
 * <p>All NithPoints transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    // These are serialized in both NithPoints and java serialization.
     ...
     * @param msg NithPoints protocol formatted byte array containing message content.
    ...
    /**
     * Used by NithPointsSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of NithPoints share. In 2011 the value of a coinbase
    ...
    public String toString(AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(NithPointsValueToFriendlyString(out.getValue()));
                s.append(" PTS");
     ...
     * features of NithPoints are developed.
     *
    ...
    synchronized Sha256Hash hashTransactionForSignature(int inputIndex, byte[] connectedScript,
        ...
        //
        //   https://en.NithPoints.it/wiki/Contracts
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in NithPoints 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // NithPoints implementation. The reference client also contains a bug in handling this case:
    ...
    @Override
    protected void NithPointsSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.NithPointsSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.NithPointsSerialize(stream);
     ...
     * This is useful in certain types of <a href="http://en.NithPoints.it/wiki/Contracts">contracts</a>, such as
